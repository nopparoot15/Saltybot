# saltybot/utils/discord_helpers.py

import re
import unicodedata

# ==== regex & maps เดิมของคุณ (ยกมาจากโปรเจ็กต์เดิม) ====
EMOJI_RE = re.compile(
    r"["
...
def canon_full(s: str) -> str:
    """แปลงชื่อให้เป็น canonical string ที่เทียบได้ (ตัด emoji/zero-width/accents/leet ฯลฯ)"""
    if not s:
        return ""
...
def discord_names_set(member) -> set[str]:
    """คืนชุดชื่อ 'canonical' ของสมาชิก จากชื่อทุกช่องบน Discord + base display"""
    names_raw = filter(None, {
        getattr(member, "nick", None),
    ...
    return {canon_full(n) for n in names_raw if n}
