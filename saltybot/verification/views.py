    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            ...
            if nick:
                ...
                # รองรับทั้ง 2 ซิกเนเจอร์ของ discord_names_set (มี/ไม่มีพารามิเตอร์ canon_full)
                try:
                    name_set = discord_names_set(interaction.user)
                except TypeError:
                    name_set = discord_names_set(interaction.user, canon_full)

                if canon_full(nick) in name_set:
                    await interaction.followup.send(
